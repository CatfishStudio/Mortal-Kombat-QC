package mkquest.assets.tutorial 
{
	import flash.system.*;
	...
	public class Tutorial extends Sprite 
	{
		...
		private function showDialogAndArrow():void
		{
			...
			if (_step == 1)
			{
				_textField = new TextField(180, 150, "Сразись со мной в турнире. \n Принимаешь вызов или сдаёшься.", "Arial", 17, 0xffffff, false);
				_textField.x = 25;
				_textField.y = 10;
				_dialog.addChild(_textField);
				
				_arrow = new Sprite();
				_arrowPosX = Constants.GAME_WINDOW_WIDTH / 2.5 - 75;
				_arrowPosY = Constants.GAME_WINDOW_HEIGHT / 2.5 - 10;
				_arrow.x = _arrowPosX;
				_arrow.y = _arrowPosY;
				bitmap = new Resource.TextureTutorialRight();
				_image = new Image(Texture.fromBitmap(bitmap));
			...
			if (_step == 2)
			{
				_textField = new TextField(180, 150, "Выбери избранного. \n Начни игру.", "Arial", 17, 0xffffff, false);
				_textField.x = 25;
				_textField.y = 10;
				_dialog.addChild(_textField);
		...
		private function animationArrowLeftRight():void
		{
			...
			if (_arrow.x == _arrowPosX) _tween.moveTo(_arrow.x - 5, _arrow.y);
			if (_arrow.x == _arrowPosX - 5) _tween.moveTo(_arrow.x + 5, _arrow.y);
		...
		{
			_tween = new Tween(_arrow, 0.5);
			if (_arrow.y == _arrowPosY) _tween.moveTo(_arrow.x, _arrow.y - 5);
			if (_arrow.y == _arrowPosY - 5) _tween.moveTo(_arrow.x, _arrow.y + 5);
